...
After DB bootstrap, an optional interactive phase prompts for workspace_roots
(project directories to pre-approve for file access). This phase requires an
interactive terminal — it is silently skipped when stdin is not a TTY (e.g.
when run from inside Claude Code or piped). Use ``--no-workspace-prompts``
to suppress it explicitly in scripts or CI.
"""
...
def _configure_workspace_roots(args: argparse.Namespace) -> None:
    ...
    4. Interactive prompt → collect paths, write if any collected.
    """
